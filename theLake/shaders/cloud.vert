...
float calc_noise(vec3 pos){
	...
	float c = 0.9 * pnoise( 0.1 * pos + vec3( time/5.0), vec3( 100.0 ) );

	float d = 0.2 * pnoise( 0.8 * pos+ vec3( time/5.0 ), vec3( 100.0 ) );
	float e = 0.15 * pnoise( 2.9 * pos+ vec3( time/5.0 ), vec3( 100.0 ) );
	return - noise + b + c + b*d  + max(d*e,0.0);
}
