...
float getBumpmap(vec3 pos){
	...
	float b = 3.0 * pnoise( 0.05 * pos + vec3( time/5.0 ), vec3( 100.0 ) );
	float c = 2.9 * pnoise( 0.1 * pos + vec3( time/5.0), vec3( 100.0 ) );

	float d = 0.5 * pnoise( 0.8 * pos+ vec3( time/5.0 ), vec3( 100.0 ) );
	float e = 0.45 * pnoise( 1.9 * pos+ vec3( time/5.0 ), vec3( 100.0 ) );
...
void main()
{
    ...
    for(int i = 0; i < 9; i++)
    {
        sampleTex[i] = vec3(texture(cloudTexture, screen_coord + offsets[i]));
    }
    vec3 col = vec3(0.0);
    for(int i = 0; i < 9; i++)
	...
	vec3 d1 =  vec3(0.001 * gl_FragCoord.z / gl_FragCoord.w);
	vec3 d2 = vec3(texture(cloudTexture, screen_coord + 0.01f*vec2(normal.x, normal.z) ));
	vec3 color = 50.0 * (d2 - d1);

	outputF = vec4(vec3(1.0), color.x);
} 