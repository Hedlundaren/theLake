...
float calc_noise(vec3 pos){
	...
	float lift_edges = 0.002*(pow(pos.x, 2.0) + 0.003*pow(-pos.z, 3.0));
...
void main() {
	...
	newPos = vec3(position.x, -(position.y + displacement - 5.0f), position.z);
	texCoord = uv;

	if(newPos.y > 0.5f){
		gl_Position = P * MV * vec4(vec3(newPos.x, 0.0f, newPos.z), 1.0f);
