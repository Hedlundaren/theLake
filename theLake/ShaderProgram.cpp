...
void ShaderProgram::updateCubeMapUniforms(MouseRotator rotator, float width, float height, float time, glm::vec3 clear_color, const int side) {
	// Uniforms
	GLint MV_Loc, P_Loc, lDir_Loc, camPos_Loc, clear_color_Loc, time_Loc = -1;
	time_Loc = glGetUniformLocation(*this, "time");
	MV_Loc = glGetUniformLocation(*this, "MV");
	...
	glm::vec4 camPos = glm::vec4(glm::vec3(0.0f), 1.0f);
	...
	glm::mat4 V = glm::lookAt(glm::vec3(camPos), point, up);
	P = glm::perspectiveFov(90.0f, static_cast<float>(width), static_cast<float>(height), 0.1f, 2000.0f);
	MV = V * M;
